...
enum RCCorrupt { CO_VIRT = 1, SEQUENCE, INDEX };
...
enum RC {
  ...
  RECORD_NOMOREIDXINMEM = (RECORD | (RCRecord::RD_NO_MORE_IDXINMEM << 8)),
  RECORD_INVALIDKEY = (RECORD | (RCRecord::RD_INVALIDKEY << 8)),
  ...
  IOERR_DELETE = (IOERR | (RCIOError::DELETE << 8)),
  ...
  /* corrupt part */
  CORRUPT_VIRT = (CORRUPT | (RCCorrupt::CO_VIRT << 8)),
  CORRUPT_SEQUENCE = (CORRUPT | (RCCorrupt::SEQUENCE << 8)),
  CORRUPT_INDEX = (CORRUPT | (RCCorrupt::INDEX << 8)),
